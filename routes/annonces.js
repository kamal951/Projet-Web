...
router.post("/", middleware.isLoggedIn, middleware.checkUserStatus, function (req, res) {
    ...
    geocoder.geocode(req.body.location, function (err, data) {
        if (data.status != 'OK') {
            req.flash("error", "Une erreur est survenue. L'annonce n'a pas pu être créée !");
            res.redirect("back");
        } else {
...
router.put("/:id", function (req, res) {
    geocoder.geocode(req.body.location, function (err, data) {
        if (data.status != 'OK') {
            req.flash("error", "Une erreur est survenue. L'annonce n'a pas pu être modifiée !");
            res.redirect("back");
        } else {
            var lat = data.results[0].geometry.location.lat;
            var lng = data.results[0].geometry.location.lng;
            ...
            Annonce.findByIdAndUpdate(req.params.id, { $set: newData }, function (err, annonce) {
                if (err) {
                    ...
                    req.flash("success", "Annonce modifiée avec succès");
